...
//! use pqcrypto_dilithium::dilithium3aes::*;
//! // or if using the pqcrypto crate:
//! // use pqcrypto::sign::dilithium3aes::*;
...
simple_struct!(
    PublicKey,
    ffi::PQCLEAN_DILITHIUM3AES_CLEAN_CRYPTO_PUBLICKEYBYTES
);
simple_struct!(
    SecretKey,
    ffi::PQCLEAN_DILITHIUM3AES_CLEAN_CRYPTO_SECRETKEYBYTES
);
...
pub struct DetachedSignature(
    ...
    [u8; ffi::PQCLEAN_DILITHIUM3AES_CLEAN_CRYPTO_BYTES],
    usize,
);
...
impl DetachedSignature {
    fn new() -> Self {
        DetachedSignature([0u8; ffi::PQCLEAN_DILITHIUM3AES_CLEAN_CRYPTO_BYTES], 0)
    }
}
...
impl primitive::DetachedSignature for DetachedSignature {
    ...
    fn from_bytes(bytes: &[u8]) -> Result<Self> {
        ...
        let expected = ffi::PQCLEAN_DILITHIUM3AES_CLEAN_CRYPTO_BYTES;
        if actual > expected {
        ...
        let mut array = [0u8; ffi::PQCLEAN_DILITHIUM3AES_CLEAN_CRYPTO_BYTES];
...
pub const fn signature_bytes() -> usize {
    ffi::PQCLEAN_DILITHIUM3AES_CLEAN_CRYPTO_BYTES
}
...
/// Generate a dilithium3aes keypair
pub fn keypair() -> (PublicKey, SecretKey) {
    #[cfg(all(enable_x86_avx2, feature = "avx2"))]
    {
        if std::is_x86_feature_detected!("avx2") {
            return gen_keypair!(PQCLEAN_DILITHIUM3AES_AVX2_crypto_sign_keypair);
        }
    }
    gen_keypair!(PQCLEAN_DILITHIUM3AES_CLEAN_crypto_sign_keypair)
}
...
pub fn sign(msg: &[u8], sk: &SecretKey) -> SignedMessage {
    ...
    {
        if std::is_x86_feature_detected!("avx2") {
            return gen_signature!(PQCLEAN_DILITHIUM3AES_AVX2_crypto_sign, msg, sk);
        }
    }
    gen_signature!(PQCLEAN_DILITHIUM3AES_CLEAN_crypto_sign, msg, sk)
}
...
pub fn open(
    ...
    {
        if std::is_x86_feature_detected!("avx2") {
            return open_signed!(PQCLEAN_DILITHIUM3AES_AVX2_crypto_sign_open, sm, pk);
        }
    }
    open_signed!(PQCLEAN_DILITHIUM3AES_CLEAN_crypto_sign_open, sm, pk)
}
...
pub fn detached_sign(msg: &[u8], sk: &SecretKey) -> DetachedSignature {
    ...
    {
        if std::is_x86_feature_detected!("avx2") {
            return detached_signature!(PQCLEAN_DILITHIUM3AES_AVX2_crypto_sign_signature, msg, sk);
        }
    }
    detached_signature!(PQCLEAN_DILITHIUM3AES_CLEAN_crypto_sign_signature, msg, sk)
}
...
pub fn verify_detached_signature(
    ...
    #[cfg(all(enable_x86_avx2, feature = "avx2"))]
    {
        if std::is_x86_feature_detected!("avx2") {
            return verify_detached_sig!(
                PQCLEAN_DILITHIUM3AES_AVX2_crypto_sign_verify,
                sig,
                msg,
                pk
            );
    ...
    verify_detached_sig!(PQCLEAN_DILITHIUM3AES_CLEAN_crypto_sign_verify, sig, msg, pk)
}
