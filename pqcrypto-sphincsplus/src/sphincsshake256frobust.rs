...
//! // use pqcrypto::sign::sphincsshake256frobust::*;
...
simple_struct!(
    PublicKey,
    ffi::PQCLEAN_SPHINCSSHAKE256FROBUST_CLEAN_CRYPTO_PUBLICKEYBYTES
);
simple_struct!(
    SecretKey,
    ffi::PQCLEAN_SPHINCSSHAKE256FROBUST_CLEAN_CRYPTO_SECRETKEYBYTES
);
...
pub struct DetachedSignature(
    ...
    [u8; ffi::PQCLEAN_SPHINCSSHAKE256FROBUST_CLEAN_CRYPTO_BYTES],
    usize,
);
...
impl DetachedSignature {
    fn new() -> Self {
        DetachedSignature(
            [0u8; ffi::PQCLEAN_SPHINCSSHAKE256FROBUST_CLEAN_CRYPTO_BYTES],
            0,
        )
    }
}
...
impl primitive::DetachedSignature for DetachedSignature {
    ...
    fn from_bytes(bytes: &[u8]) -> Result<Self> {
        ...
        let expected = ffi::PQCLEAN_SPHINCSSHAKE256FROBUST_CLEAN_CRYPTO_BYTES;
        if actual > expected {
        ...
        let mut array = [0u8; ffi::PQCLEAN_SPHINCSSHAKE256FROBUST_CLEAN_CRYPTO_BYTES];
...
/// Generate a sphincs-shake-256f-robust keypair
pub fn keypair() -> (PublicKey, SecretKey) {
    #[cfg(all(enable_x86_avx2, feature = "avx2"))]
    {
        if std::is_x86_feature_detected!("avx2") {
            return gen_keypair!(PQCLEAN_SPHINCSSHAKE256FROBUST_AVX2_crypto_sign_keypair);
        }
    }
...
pub fn sign(msg: &[u8], sk: &SecretKey) -> SignedMessage {
    ...
    {
        if std::is_x86_feature_detected!("avx2") {
            return gen_signature!(PQCLEAN_SPHINCSSHAKE256FROBUST_AVX2_crypto_sign, msg, sk);
        }
    }
    gen_signature!(PQCLEAN_SPHINCSSHAKE256FROBUST_CLEAN_crypto_sign, msg, sk)
}
...
pub fn open(
    ...
    {
        if std::is_x86_feature_detected!("avx2") {
            return open_signed!(PQCLEAN_SPHINCSSHAKE256FROBUST_AVX2_crypto_sign_open, sm, pk);
        }
    }
    open_signed!(
